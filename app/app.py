...
def list_events():
    ...
    db = get_db()
    cur = db.cursor()
    query = '''SELECT events.*, users.name as organizer_name, users.college_name as organizer_college FROM events LEFT JOIN users ON events.organizer_id = users.id WHERE date >= date("now")'''
    params = []
    if college_name:
    ...
    # Filter events for club_admin to only show their own college
    user_role = request.args.get('role')
    user_id = request.args.get('user_id')
    if user_role == 'club_admin' and user_id:
        # Get club admin's college
        cur.execute('SELECT college_name FROM users WHERE id=? AND role="club_admin"', (user_id,))
        admin = cur.fetchone()
        if admin:
            admin_college = admin['college_name']
            events = [e for e in events if e.get('organizer_college') == admin_college]
...
def my_registrations(student_id):
    ...
    db = get_db()
    cur = db.cursor()
    cur.execute('''SELECT events.*, registrations.registration_time FROM registrations
...
def submit_feedback():
    ...
    cur.execute('SELECT * FROM attendance WHERE event_id=? AND student_id=?', (event_id, student_id))
    if not cur.fetchone():
        return jsonify({'error': 'Feedback only allowed after attendance.'}), 403
...
# Get registered students for an event (for attendance marking)
@app.route('/event_registrations/<int:event_id>', methods=['GET'])
@jwt_required
def event_registrations(event_id):
    # Verify user is club admin
    if request.current_user['role'] != 'club_admin':
        return jsonify({'error': 'Only club admins can access event registrations.'}), 403
